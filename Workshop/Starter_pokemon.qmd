...
3.  Visualize the distribution of Pokemon's statuses (attack ,defense, sp_attack, sp_defense, speed and weight_kg) by type1 of Pokemon? What can you conclude?
...
We want to develop a machine learning model to predict the Pokemon type based on their statuses. Select only the "type1", "attack" ,"defense", "sp_attack", "sp_defense", "speed" and "weight_kg" columns from the table. Remove any NAs present in the data using na.omit. Convert the type1 column to factor type.
...
1.  We will use random forest with 10 fold cross-validation approach which you need to define with function trainControl. Inside function *train* from caret package set the method argument to *ranger*, number of trees to 100 and importance to "permutation".
